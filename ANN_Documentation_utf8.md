# Hate Speech Detection (Filipino-English) â€” Project Documentation
...
## 3. Model Design
- Architecture: `rnn_model.py` â†’ `BiLSTMHateSpeechClassifier`
...
- Forward pass:
  ...
  - Concatenate final forward/backward hidden states â†’ dropout â†’ dense â†’ logits
- Parameter Count: printed in notebook (Cell 13) via `count_parameters()`

## 4. Training Setup
- Files: `train_bilstm.py`, Notebook Cells 15â€“18
...
- Error Analysis:
  ...
  - Vocabulary coverage: English polite phrases map partially to `<UNK>` â†’ misclassification
  - Threshold tuning helps: moving from 0.5 â†’ 0.75 reduces false positives

## 7. Deployment (GUI)
- App: `social_media_app.py` and `gui_app.py`
- Features:
  - Real-time text preprocessing + inference
  - Adjustable threshold slider (recommended 0.7â€“0.8 for current model)
...
3. Use `Ctrl+Shift+P` â†’ "Markdown PDF: Export (pdf)".
